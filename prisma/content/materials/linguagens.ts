...
**Começando do zero:** "Escolas literárias" são apenas *grupos de escritores que viveram na mesma época e escreviam de um jeito parecido*, reagindo ao mundo à sua volta. É como a moda ou a música: cada época tem o seu estilo. O Romantismo era sonhador e idealizava o amor e a pátria; o Realismo veio depois, mais crítico e "pé no chão"; o Modernismo quebrou as regras e trouxe o Brasil real e a fala do dia a dia para dentro dos livros. No ENEM, você não precisa decorar datas — basta ler o trecho e perceber *o jeitão* dele.
...
**Começando do zero:** Se você escolher Espanhol, são 5 questões que, na prática, são *interpretação de texto* — ninguém vai pedir para você conjugar verbos. O maior perigo são os "falsos amigos": palavras que parecem com o português, mas significam outra coisa (por exemplo, *embarazada* não é "envergonhada", é **grávida**). O segredo é não travar tentando entender cada palavra: capte a ideia geral pelo contexto, do mesmo jeito que você entende o sentido de uma conversa mesmo sem ouvir cada palavrinha.
...
**Começando do zero:** São 5 questões de Inglês para quem escolhe essa língua, e todas são *interpretação de texto* — não é prova de tradução nem de gramática decorada. Muitas palavras são parecidas com o português (as "cognatas", como *information* e *important*), e o resto você deduz pelo contexto. Foque em captar a **mensagem principal** e a intenção do texto, do mesmo jeito que se entende o recado de uma placa ou de uma música mesmo sem dominar o idioma inteiro.
...
**Começando do zero:** Pode parecer estranho "Educação Física" cair numa prova de leitura, mas aqui o assunto é encarado como *cultura e cidadania*: esportes, danças, lutas e brincadeiras são vistos como formas de expressão de um povo e como questão de saúde e inclusão. Pense na capoeira (que é luta, dança e história ao mesmo tempo) ou no direito de uma pessoa com deficiência praticar esporte. A prova quer que você enxergue o corpo e o movimento como algo social e cultural — não só como exercício de academia.
...
**Começando do zero:** Interpretar um texto é simplesmente *entender de verdade o que ele diz* — e isso não é só ler as palavras, é captar a ideia, a intenção e o que está nas entrelinhas. É a habilidade que mais rende na prova inteira, porque toda questão, de qualquer matéria, começa com um texto. O truque é sempre o mesmo: ler primeiro a pergunta (para saber o que procurar), voltar ao texto e responder com base no que *está escrito ali* — nunca no seu palpite ou na sua opinião pessoal.
...
**Começando do zero:** Todo texto pertence a um *gênero*, que é o formato que ele assume conforme a função que cumpre no dia a dia: uma receita de bolo, uma notícia de jornal, uma bula de remédio, um meme, um poema — cada um tem a sua "cara" e serve para uma coisa. Não confunda gênero com *tipo textual*, que é a estrutura por dentro (narrar, descrever, argumentar). Um mesmo gênero pode misturar vários tipos: uma propaganda descreve o produto E tenta convencer você a comprá-lo. A pegadinha da banca mora exatamente nessa diferença entre gênero e tipo.
...
**Começando do zero:** Aqui a "linguagem" não é feita de palavras, e sim de *imagens, sons e movimentos*: uma pintura, uma escultura, um grafite, uma música, uma peça de teatro. Toda obra de arte quer **comunicar** algo — uma emoção, uma crítica, a identidade de um povo. Patrimônio cultural é aquilo que uma sociedade guarda como memória, seja um prédio histórico (patrimônio material) ou uma festa tradicional e uma receita passada de geração em geração (patrimônio imaterial). O ENEM pede que você "leia" a obra: o que ela diz e por quê.
...
**Começando do zero:** Variação linguística é o fato simples de que *nem todo mundo fala igual*: muda de região para região ("mandioca", "aipim" e "macaxeira" são a mesma coisa), muda de grupo social e muda conforme a situação (você não fala com o chefe do mesmo jeito que fala com um amigo). A ideia mais importante — e a que o ENEM defende — é que não existe jeito "certo" ou "errado" de falar em termos absolutos: existe o jeito **adequado** a cada situação. Tratar a fala popular ou regional como "erro" ou "ignorância" é preconceito linguístico, e isso a prova rejeita.
...
**Começando do zero:** Este tema é a linguagem no *mundo digital*: os posts, memes, podcasts, comentários e prints de rede social que fazem parte do nosso dia a dia. Cada um desses formatos tem o seu jeito próprio de comunicar, misturando texto, imagem, emoji e hashtag. Também entram aqui os problemas desse mundo, como as fake news (notícias falsas que se espalham rápido) e as "bolhas" (quando só enxergamos quem pensa igual a nós). A prova mostra um desses conteúdos e pergunta *que efeito ele produz* ou *para que ele serve*.
...
**Começando do zero:** Figuras de linguagem são *jeitos criativos de dizer as coisas* que fogem do sentido literal para dar mais força ou beleza à frase — e você usa várias sem nem perceber. Quando alguém diz "estou morrendo de fome", ninguém está morrendo de verdade: é um exagero (hipérbole). Dizer que "aquele político é uma raposa" não fala de um animal: é uma metáfora. Elas aparecem muito em poemas, músicas e propagandas, e a prova pede para você reconhecer a figura e, principalmente, entender *o efeito de sentido* que ela cria.
...
**Começando do zero:** Intertextualidade é quando *um texto conversa com outro* — cita, imita, relembra ou brinca com uma obra que já existe. Isso acontece o tempo todo: uma propaganda que recria a Mona Lisa, um meme baseado num verso famoso, uma música que responde a outra. A distinção que mais cai é entre **paráfrase** (quando o texto novo concorda e repete a ideia com outras palavras) e **paródia** (quando ele transforma o original para criticar ou fazer humor). Pergunte-se sempre: o texto novo está *reforçando* ou *zoando/contestando* o texto original?
...
**Começando do zero:** Toda vez que a gente se comunica, tem uma *intenção* — e as "funções da linguagem" são só uma lista de seis intenções possíveis. Às vezes queremos informar (uma notícia), às vezes expressar um sentimento ("estou tão feliz!"), às vezes convencer alguém a fazer algo (uma propaganda: "compre já!"), às vezes só manter o contato ("alô? tá me ouvindo?"), às vezes explicar a própria língua (um dicionário) e às vezes caprichar na forma bonita das palavras (um poema). A prova mostra um texto e pergunta qual dessas intenções **predomina** nele.
...
**Começando do zero:** No ENEM, gramática não cai como regra decorada ("o que é um pronome?"); ela cai a *serviço do sentido do texto*. O foco é a **coesão**: como as partes de um texto se amarram e se encaixam para fazer sentido. Isso acontece principalmente por dois recursos: os conectivos (palavrinhas como "mas", "porque", "portanto", que ligam as ideias e mostram se é oposição, causa ou conclusão) e a referenciação (usar um pronome como "ele" ou "isso" para não ficar repetindo a mesma palavra). A prova pergunta *que relação* ou *que ligação* esses recursos criam no texto.
